...
static int	count_cols(t_vars *vars, char *map)
{
	int		col_count;
	char	*line;
	char	**dummy_ptr;
	...
	fd = open(map, O_RDONLY);
	if (fd < 0)
		return (handle_err(vars, 11), 0);
	col_count = 0;
	line = get_next_line(fd, 1);
	if (!line)
		return (free(line), handle_err(vars, 11), 0);
	...
	return (free(*spltd_line), close(fd), free(dummy_ptr), col_count);
}
...
static void	alloc_z_clr(t_matrix *matrix)
{
	...
	while (y < matrix->max_y)
	{
		...
		while (x < matrix->max_x)
		{
			...
			if (!matrix->matrix_ptr[y][x])
				return (handle_err(0, 4), deallocate_data(matrix));
			x++;
		}
		y++;
	}
...
void	alloc_store_matrix(char *map, t_vars *vars)
{
	...
	while (i < vars->matrix.max_y)
	{
		...
		if (!vars->matrix.matrix_ptr[i++])
			return (deallocate_data(&(vars->matrix)), handle_err(0, 3));
	}
